function generateMicroscopeEpiConfigs
    
    fileID = fopen(which('DufourLab_Scope_Channel_Fluo.cfg'),'w');
        ...
        for k2 = 1:numel(filters)
            for k3 = 1:numel(binning)
                for k4 = 1:numel(exposure)
                    for k5 = 1:numel(sola)
                        name = sprintf('EPI_%s_%s_%s_%dms_%dsola',objectives{k1},filters{k2},binning{k3},exposure(k4),sola(k5));
                        fprintf(fileID,'\n# Preset: %s\n',name);
                        fprintf(fileID,'ConfigGroup,Channel,%s,Andor Zyla 4.2,Binning,%s\n',name,binning{k3});
                        fprintf(fileID,'ConfigGroup,Channel,%s,Arduino-Switch,State,2\n',name);
                        ...
                        fprintf(fileID,'ConfigGroup,Channel,%s,Sola,State,1\n',name);
                        fprintf(fileID,'ConfigGroup,Channel,%s,Sola,White_Enable,1\n',name);
                        fprintf(fileID,'ConfigGroup,Channel,%s,Sola,White_Level,%d\n',name,sola(k5));
                    end
                end
            end
        end
