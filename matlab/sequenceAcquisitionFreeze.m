function file = sequenceAcquisitionFreeze(mmc, file, runtime, config, fps, bit8, crop, freeze, moretime)
...
if ~(mmc.getProperty('TIFilterBlock1','State') == '0')
    error('The filter block need to be set to DAPI for polymerization to work!\n');
end
if ismember('Sola',string(mmc.getLoadedDevices.toArray()))
    mmc.setProperty('Sola','State',1);
    mmc.setProperty('Sola','White_Enable',1);
    mmc.setProperty('Sola','White_Level',100);
else
    error('The EPI light source (Sola) is not loaded! Turn it ON and re-initialize the microscope.\n');
end
...
nFreezing = ceil(freeze*1000/interval)+skip_frame-1;
nMoreImages = ceil(moretime*1000/interval)+skip_frame-1;

circular_buffer = max(100,min(max_buff_size_mb,((nImages+nFreezing+nMoreImages)+skip_frame+10)*w*h*mmc.getBytesPerPixel/(1024^2)));
...
fprintf('Freezing for %d seconds\n', freeze);
fprintf('Capturing %d more images for %d seconds\n', ceil(nMoreImages/skip_frame),moretime);
...
mmc.startSequenceAcquisition((nImages+nFreezing+nMoreImages)+skip_frame-1,0,1);
tic1=tic;
...
while mmc.isSequenceRunning && ~mmc.isBufferOverflowed
    currentNim = mmc.getRemainingImageCount();
    fprintf('\b\b\b\b%0.3d%%',ceil(100*currentNim/((nImages+nFreezing+nMoreImages)+skip_frame)));
    
    if freezeON && currentNim >= nImages+nFreezing-1
        mmc.setProperty('Arduino-Switch','Blanking Mode','On');
        mmc.setProperty('Arduino-Switch','State',1);
        fprintf('  more      ');
        freezeON = false;
        
    elseif ~freezeON && currentNim >= nImages && currentNim < nImages+nFreezing-1
...
Nremaining = mmc.getRemainingImageCount();
frozenImg = 0;
Nfrozen = 0;
backgroundImg = 0;
Nbg = 0;
...
while Nremaining>0
    ...
    if Nremaining == nFreezing+nMoreImages
        imagesBeforeFreeze = nImages_actual+1;
    elseif Nremaining == nMoreImages
        imagesDuringFreeze = nImages_actual-imagesBeforeFreeze+1;
    end
    ...
    if mod(i-sync_seq, skip_frame) == 0
        ...
        if bit8 == 1 && realbit ~= 8
         ...
         if Nremaining > nFreezing+nMoreImages-1
             backgroundImg = backgroundImg + double(img);
            Nbg = Nbg + 1;
         end
        
        if Nremaining < nMoreImages
            frozenImg = frozenImg + double(img);
            Nfrozen = Nfrozen + 1;
        end
...
imagesAfterFreeze = nImages_actual - imagesBeforeFreeze - imagesDuringFreeze;

fclose(fid);
fprintf('%d images\n\n', nImages_actual);

%%
fid2 = fopen(strcat(file,'.background'), 'w');
if bit8 == 1 && realbit ~= 8
    fwrite(fid2, uint8(backgroundImg/Nbg), 'uint8');
else
    fwrite(fid2, uint8(backgroundImg/Nbg), pixelType);
end

fclose(fid2);

%%
fid3 = fopen(strcat(file,'.lastimage'), 'w');
if bit8 == 1 && realbit ~= 8
    fwrite(fid3, uint8(frozenImg/Nfrozen), 'uint8');
else
    fwrite(fid3, uint8(frozenImg/Nfrozen), pixelType);
end
...
metadata.add('SequenceAcquisition', 'NImagesDuringFreeze', ...
    sprintf('%d', imagesDuringFreeze));
metadata.add('SequenceAcquisition', 'NImagesAfterFreeze', ...
    sprintf('%d', imagesAfterFreeze));
